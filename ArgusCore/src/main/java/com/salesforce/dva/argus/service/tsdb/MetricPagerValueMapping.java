...
@ThreadSafe
public class MetricPagerValueMapping extends MetricPager {
	
	private ValueMapping mapping;
	private MetricPageScanner pageScanner;

	public MetricPagerValueMapping(List<MetricScanner> scanners, Long chunkTime, ValueMapping mapping) {
		SystemAssert.requireArgument(scanners != null, "Cannot return value mapping of null scanners.");
		SystemAssert.requireArgument(scanners.size() == 1, "Value mappings can only be performed on a single scanner.");
		...
		pageScanner = new MetricPageDatapointScanner(new HashMap<>(),
				PageScannerShifts.getValueMappingShift(scanners.get(0).getShift(), mapping, constants), start.get(), end.get());
	...
	public MetricPagerValueMapping(List<MetricScanner> scanners, Long chunkTime, ValueMapping mapping, List<String> constants) {
		SystemAssert.requireArgument(scanners != null, "Cannot return value mapping of null scanners.");
		SystemAssert.requireArgument(scanners.size() == 1, "Value mappings can only be performed on a single scanner.");
		...
		this.chunkTime = new AtomicLong(chunkTime);
		this.mapping = mapping;
		numChunks = this.chunkTime.get() == 0 ? 1 : (int) Math.ceil((end.get() - start.get()) / ((double) this.chunkTime.get()));
		lastEndTimestamp = new AtomicLong(start.get());
		inUse = new AtomicBoolean(true);
		complete = new AtomicBoolean(false);
		pageScanner = new MetricPageDatapointScanner(new HashMap<>(),
	...
	protected void performTransform() {
		...
		Map<Long, Double> result = constants == null ? mapping.mappingToPager(scanners.get(0), prevShift.shift(startTime), prevShift.shift(endTime)) :
			mapping.mappingToPager(scanners.get(0), constants, prevShift.shift(startTime), prevShift.shift(endTime));
