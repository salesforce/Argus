...
public class RangeValueReducerOrMappingScannerTest extends AbstractTest {
	...
	@Test
	public void testReduceWithoutNull() {
		...
		RangeValueReducerOrMapping reducer = new RangeValueReducerOrMapping();
		
		for (int i = 0;  i < metrics.size(); i++) {
			List<Double> vals = new ArrayList<>(metrics.get(i).getDatapoints().values());
			Double expected = reducer.reduce(vals);
	...
	@Test
	public void testReduceWithSomeNull() {
