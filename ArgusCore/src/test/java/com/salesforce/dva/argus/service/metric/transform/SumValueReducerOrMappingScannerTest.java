...
public class SumValueReducerOrMappingScannerTest extends AbstractTest {
	...
	private Map<Long, Double> mappingWithNullTolerance(Metric m, List<String> constants) {
		...
		for (Map.Entry<Long, Double> entry : m.getDatapoints().entrySet()) {
			...
			transformedDatapoints.put(entry.getKey(), entry.getValue() + addend);
		}
		return transformedDatapoints;
	}
	
	@Test
	public void testReduceCall() {
		...
		SumValueReducerOrMapping redMap = new SumValueReducerOrMapping();
	...
	@Test
	public void testMappingWithoutNull() {
	...
	public void testMappingWithNull() {
		...
		for (int i = 0; i < metrics.size(); i++) {
			// Map<Long, Double> expected = redMap.mapping(metrics.get(i).getDatapoints(), constants); -> metrics version has no null tolerance
