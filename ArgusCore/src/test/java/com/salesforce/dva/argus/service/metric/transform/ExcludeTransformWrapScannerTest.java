...
public class ExcludeTransformWrapScannerTest extends AbstractTest {
	...
	@Test
	public void testTransformWithRegexSomeExclusionsApply() {
		...
		for (int i = 0; i < metrics.size(); i++) {
			metrics.get(i).setMetric(TEST_METRIC + i); 
		}
		...
		ExcludeTransformWrap transform = new ExcludeTransformWrap();
	...
	@Test
	public void testTransformWithRegexAllAreExcluded() {
	...
	@Test
	public void testTransformWithRegexNoneAreExcluded() {
		...
		List<Metric> metrics = createRandomMetrics(null, null, 10);
		for (int i = 0; i < metrics.size(); i++) {
			metrics.get(i).setMetric(TEST_METRIC + "1"); 
		}
		List<MetricQuery> queries = toQueries(metrics);
		List<MetricScanner> scanners = new ArrayList<>();
	...
	@Test
	public void testDisposeUnusedScanners() {
	...
	public void testPager() {
		...
		if (metrics.get(0).getDatapoints().size() == 1) {
			Map<Long, Double> dps = new HashMap<>();
			dps.put(System.currentTimeMillis(), random.nextDouble() * 100);
			metrics.get(0).addDatapoints(dps);
		}
		List<MetricQuery> queries = toQueries(metrics);
		Long max = null;
		Long min = null;
		...
		Transform transform = new ExcludeTransformWrap();
		List<String> constants = new ArrayList<>();
		constants.add(TEST_INCLUDE_REGEX);
