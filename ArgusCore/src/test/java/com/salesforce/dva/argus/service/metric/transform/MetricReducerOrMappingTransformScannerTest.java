...
public class MetricReducerOrMappingTransformScannerTest extends AbstractTest {
	...
	@Test
	public void testTransformNoConstantsWithSumReducing() {
		...
		ValueReducerOrMapping redMap = new SumValueReducerOrMapping();
		MetricReducerOrMappingTransform transform = new MetricReducerOrMappingTransform(redMap);
	...
	@Test
	public void testTransformWithConstantsWithDivideReducing() {
		...
		List<Metric> expected = transform.mapping(metrics, constants);
		List<Metric> actual = transform.mappingScanner(scanners, constants);
	...
	public void testPager() {
		...
		Transform transform = new MetricReducerOrMappingTransform(new SumValueReducerOrMapping());
		List<String> constants = new ArrayList<>();
		constants.add("" + random.nextDouble() * 10);	
		...
		Transform transform2 = new MetricReducerOrMappingTransform(new DiffValueReducerOrMapping());		
		
		expected = transform2.transform(metrics);
		MetricPager stream2 = new MetricPagerTransform(scanners, chunkTime, transform2);			
		
		chunk = random.nextInt(stream2.getNumberChunks());
		start = stream2.getStartTime() + (chunk) * chunkTime;
		end = Math.min(start + chunkTime, stream.getEndTime());
		resChunk = stream2.getMetricChunk(chunk);
		for (Metric m : expected) {
			TreeMap<Long, Double> dps = new TreeMap<>(m.getDatapoints());
			if (!dps.subMap(start, end + 1).isEmpty()) {
				assert(resChunk.contains(m));
		...
		act = stream2.getMetricChunk(0);
		for (int j = 1; j < stream2.getNumberChunks(); j++) {
			List<Metric> b = stream2.getMetricChunk(j);
			for (Metric m : b) {
				act.get(act.indexOf(m)).addDatapoints(m.getDatapoints());
			}
		}
