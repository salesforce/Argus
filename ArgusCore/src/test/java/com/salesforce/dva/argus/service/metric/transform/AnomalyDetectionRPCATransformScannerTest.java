...
public class AnomalyDetectionRPCATransformScannerTest extends AbstractTest {
	...
	@Test
	public void testTransformShortSeasons() {
		...
		AnomalyDetectionRPCATransform transformMetrics = new AnomalyDetectionRPCATransform();
		AnomalyDetectionRPCATransform transformScanners = new AnomalyDetectionRPCATransform();
		...
		for (int i = 0; i < metrics.size(); i++) {
			...
			if (metrics.get(i).getDatapoints().size() > 1) {	// need this to calculate frequency
	...
	@Test
	public void testTransformLongSeasons() {
	...
	public void testPager() {
		...
		Transform transform = new AnomalyDetectionRPCATransform();
		List<String> constants = new ArrayList<>();
		constants.add("2h");
				...
				if (!dps.subMap(start, end + 1).isEmpty()) {
					...
					for (int j = 0; j < dps.subMap(start, end + 1).size(); j++) {
						for (Long time : dps.subMap(start, end + 1).keySet()) {
							assert(Math.abs(dps.get(time) - resChunk.get(index).getDatapoints().get(time)) < epsilon);
						}
					}
				} else {
					assert(resChunk.contains(m));
