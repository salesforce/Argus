...
public class ScaleValueReducerOrMappingScannerTest extends AbstractTest {
	...
	private Map<Long, Double> mappingWithNullTolerance(Metric m, List<String> constants) {
		double multiplicand = Double.parseDouble(constants.get(0));
		Map<Long, Double> scaleDatapoints = new HashMap<>();
		for (Map.Entry<Long, Double> entry : m.getDatapoints().entrySet()) {
			if (entry.getValue() != null) {
				scaleDatapoints.put(entry.getKey(), entry.getValue() * multiplicand);
			}
		}
	...
	public void testReduceNoNull() {
		...
		ScaleValueReducerOrMapping redMap = new ScaleValueReducerOrMapping();
	...
	public void testReduceWithNull() {
		...
		for (Metric m : metrics) {
			...
			Long timestmap = Collections.min(dps.keySet()) + (Collections.max(dps.keySet()) - Collections.min(dps.keySet())) / 2;
			dps.put(timestmap, null);
			m.setDatapoints(dps);
		}
		List<MetricQuery> queries = toQueries(metrics);
		List<MetricScanner> scanners = new ArrayList<>();
	...
	public void testMappingNoNull() {
		...
		for (int i = 0; i < metrics.size(); i++) {
			...
			constants.set(0, "" + random.nextDouble() * 10);
		}
	}
	
	@Test
	public void testMappingWithNull() {
	...
	public void testPager() {
		...
		ValueReducerOrMapping redMap = new ScaleValueReducerOrMapping();
		List<String> constants = new ArrayList<>();
		constants.add("3.0");
