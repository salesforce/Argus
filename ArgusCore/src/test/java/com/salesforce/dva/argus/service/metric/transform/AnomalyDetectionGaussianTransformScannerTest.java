...
public class AnomalyDetectionGaussianTransformScannerTest extends AbstractTest {
	...
	@Test
	public void testTransformWithDensityTransformImplementation() {
	...
	public void testPager() {
		...
		for (int i = 0; i < metrics.size(); i++) {
			...
			List<Metric> expected = transform.transform(l);
			Long chunkTime = (queries.get(i).getEndTimestamp() - queries.get(i).getStartTimestamp()) / 7;
			MetricPager stream = new MetricPagerTransform(ms, chunkTime, transform);
