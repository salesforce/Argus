...
public class MetricMappingTransformScannerTest extends AbstractTest {
	...
	@Test
	public void testTransformScannerNoConstantsNoNullWithIntegralValueMapping() {
		...
		ValueMapping map =  new IntegralValueMapping();
		MetricMappingTransform transform = new MetricMappingTransform(map);
	...
	@Test
	public void testTransformScannerNoConstantsWithNullWithIntegralValueMapping() {
	...
	@Test
	public void testTransformScannerWithConstantsNoNullWithCullAboveValueMappingPercentile() {
		...
		ValueMapping map =  new CullAboveValueMapping();
		MetricMappingTransform transform = new MetricMappingTransform(map);
	...
	@Test
	public void testTransformScannerWithConstantsWithNullWithCullAboveValueMappingPercentile() {
	...
	public void testPager() {
		...
		Transform transform = new MetricMappingTransform(new CullAboveValueMapping());
		List<String> constants = new ArrayList<>();
		constants.add("" + random.nextDouble() * 100);
		constants.add("percentile");		
		...
		transform = new MetricMappingTransform(new IntegralValueMapping());		
		
		expected = transform.transform(metrics);
		stream = new MetricPagerTransform(scanners, chunkTime, transform);			
		
		chunk = random.nextInt(stream.getNumberChunks());
		start = stream.getStartTime() + (chunk) * chunkTime;
		end = Math.min(start + chunkTime, stream.getEndTime());
		resChunk = stream.getMetricChunk(chunk);
		for (Metric m : expected) {
			TreeMap<Long, Double> dps = new TreeMap<>(m.getDatapoints());
			if (!dps.subMap(start, end + 1).isEmpty()) {
				assert(resChunk.contains(m));
