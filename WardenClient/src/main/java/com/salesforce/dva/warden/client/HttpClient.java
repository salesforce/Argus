...
public class HttpClient
{
     ...
     *             If an error occurs.
     */
    public HttpClient( String endpoint, int maxConn, int timeout, int reqTimeout )
    {
        ...
        try
        {
            ...
            String routePath = endpoint.substring( 0,
                                                   endpoint.lastIndexOf( ":" ) );
            HttpHost host = new HttpHost( routePath, port );
            RequestConfig defaultRequestConfig =
                RequestConfig.custom(  ).setConnectionRequestTimeout( reqTimeout ).setConnectTimeout( timeout ).build(  );

            connMgr.setMaxPerRoute( new HttpRoute( host ),
                                    maxConn / 2 );
    ...
    HttpResponse executeHttpRequest( RequestType requestType, String url, StringEntity entity )
                             throws IOException
    {
