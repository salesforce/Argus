...
public class AnomalyDetectionGaussianTransformDensityScannerTest extends AbstractTest {
	...
	@Test
	public void testCalculateAnomalyScore() {
		...
		AnomalyDetectionGaussianDensityTransform transformMetric = new AnomalyDetectionGaussianDensityTransform();
		AnomalyDetectionGaussianDensityTransform transformScanner = new AnomalyDetectionGaussianDensityTransform();
		...
		for (int i = 0; i < metrics.size(); i++) {
			...
			transformMetric.transform(mList);
			transformScanner.transformScanner(msList);
			
			double value = random.nextDouble() * 100.0;
			
			double expected = transformMetric.calculateAnomalyScore(value);
			double actual = transformScanner.calculateAnomalyScore(value);
			
			double epsilon = Math.pow(1, -6);
			
			assert(Math.abs(expected - actual) < epsilon || ((transformMetric.variance == 0.0) && (transformScanner.variance == 0.0)));	// will have floating point errors
		}
	}
}
