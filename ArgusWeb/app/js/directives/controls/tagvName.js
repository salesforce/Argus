...
angular.module('argus.directives.controls.tagvName', [])
.directive('agTagvName', ['CONFIG', '$routeParams', 'SearchService', function(CONFIG, $routeParams, SearchService) {
	return {
		restrict: 'EA',
		scope: {
		...
		'<script type="text/ng-template" id="tagvTemplate.html">'+
				'<ul class="dropdown-menu viewMetricsUl"'+
						...
						'ag-infinite-scroll="scope.$parent.$parent.loadMore(scope.matches, \'loadingTagv\')"'+
				'>'+
		...
		link: function(scope, element, attributes, dashboardCtrl) {
			dashboardCtrl.updateControl('tagv', scope.ctrlVal, 'agTagvName');
			scope.$watch('ctrlVal', function(newValue){
				dashboardCtrl.updateControl('tagv', newValue, 'agTagvName', true);
			});
		}
	};
}]);